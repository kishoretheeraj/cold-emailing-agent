...
> and never place it in a local .env file** — this is the one hard rule in this entire plan that
> must never be worked around, even to make a test pass.
...
- [x] **Step 1: Add the job-pick step**
...
- [ ] **Step 2: Manually verify the workflow end-to-end**  <!-- blocked: this environment does not have the capability to perform this step. It requires a human-operated live run against production: a real Supabase write, real Anthropic spend, and job_pick.py's zero-tap resume_agent build. An unattended runner must not execute it -- leave unchecked and stop. The repo owner runs this by hand. -->
...
- [ ] **Step 2: Run it once, then fill in real values**  <!-- blocked: this environment does not have the capability to perform this step. It writes a real row to the production `prompts` table, and the values are the repo owner's own EEO/work-authorization answers, which no agent may supply or guess. Leave unchecked and stop. -->
...
- [ ] **Step 5: Live smoke test — human only, not build-continue.yml**  <!-- blocked: this environment does not have the capability to perform this step. It requires a human driving a real browser against real third-party employer application pages. Leave unchecked and stop. -->
...
- [ ] **Step 5: Live smoke test — human only, not build-continue.yml**  <!-- blocked: this environment does not have the capability to perform this step. It requires a human driving a real browser against real employer pages, and is additionally blocked on the browser-use API mismatch documented in root CLAUDE.md. Leave unchecked and stop. -->
...
- [x] **Step 1: Read the existing trigger-agent route for the exact pattern to match**
...
- [x] **Step 1: Write the preview workflow**
...
- [x] **Step 2: Write the submit workflow**
...
- [x] **Step 3: Verify `GITHUB_DISPATCH_TOKEN` already covers this new workflow**
...
- [ ] **Step 4: Manually trigger the preview workflow once to confirm it works end-to-end**  <!-- blocked: this environment does not have the capability to perform this step. It triggers a real preview pass against production (real Supabase writes, real Anthropic spend) and drives real employer pages. Leave unchecked and stop. -->
...
- [ ] **Step 5: Manually trigger the submit workflow once, against a real approved row — human only**  <!-- blocked: this environment does not have the capability to perform this step, and must never attempt it. It submits a REAL job application to a REAL employer on the repo owner's behalf -- an irreversible real-world action requiring their explicit approval tap. Leave unchecked and stop. -->
...
- [x] **Step 2: Update contact-manager/CLAUDE.md**
...
- [x] **Step 3: Update the buildout master spec**
...
- [x] **Step 5: Final full-suite check**
