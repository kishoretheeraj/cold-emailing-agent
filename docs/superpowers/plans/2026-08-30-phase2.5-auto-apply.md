...
- [ ] **Step 2: Manually verify the workflow end-to-end**  <!-- blocked: not run -- `gh workflow run jobright_pull.yml` triggers a real production Supabase write and real Anthropic spend. Deferred to the user, post-merge. -->
...
- [ ] **Step 2: Run it once, then fill in real values**  <!-- blocked: not run -- writes a real row to the production `prompts` table, and only the user should fill in their own EEO/work-authorization answers. Deferred to the user. -->
...
- [ ] **Step 5: Live smoke test — human only, not build-continue.yml**  <!-- blocked: not run -- human-only live smoke test against real Ashby/Greenhouse/Lever application pages with a real browser. -->
...
- [ ] **Step 5: Live smoke test — human only, not build-continue.yml**  <!-- blocked: not run -- human-only live smoke test; also blocked on the browser-use API mismatch documented in root CLAUDE.md. -->
...
- [ ] **Step 4: Manually trigger the preview workflow once to confirm it works end-to-end**  <!-- blocked: not run -- triggers a real unattended preview pass (real Supabase writes, real Anthropic spend). Deferred to the user, post-merge. -->
...
- [ ] **Step 5: Manually trigger the submit workflow once, against a real approved row — human only**  <!-- blocked: not run -- would submit a REAL job application to a REAL employer. Human-only by design; never run this from an agent session. -->
